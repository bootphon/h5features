...
std::vector<double> utils::generate_vector(std::size_t size) {
...
std::vector<double> utils::generate_range(double start, double stop) {
  std::vector<double> vec(stop - start);
  std::iota(vec.begin(), vec.end(), start);
  return vec;
}

h5features::features utils::generate_features(std::size_t size, std::size_t dim) {
  return {generate_vector(size * dim), dim};
}

h5features::times utils::generate_times(std::size_t size, h5features::times::format format) {
  switch (format) {
...
h5features::item utils::generate_item(const std::string &name, std::size_t size, std::size_t dim, bool properties,
                                      h5features::times::format format) {
  ...
  return {name, generate_features(size, dim), generate_times(size, format), props};
}
