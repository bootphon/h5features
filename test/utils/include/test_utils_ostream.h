...
namespace std {
template <class T1, class T2> std::ostream &boost_test_print_type(std::ostream &ostr, const std::pair<T1, T2> &v) {
  ostr << v.first << ": " << v.second;
  return ostr;
}

template <class T> std::ostream &boost_test_print_type(std::ostream &ostr, const std::vector<T> &v) {
  ostr << "[";
  if (v.size() > 0) {
    ostr << v[0];
  }
...
template <class T> std::ostream &boost_test_print_type(std::ostream &ostr, const std::set<T> &v) {
...
std::ostream &boost_test_print_type(std::ostream &ostr, const h5features::times &v) {
  ostr << "times (" << v.size() << "): [" << v.start() << ", ..., " << v.stop() << "]";
  return ostr;
}
