...
BOOST_AUTO_TEST_CASE(test_validate_1) {
  BOOST_CHECK_EXCEPTION(h5features::features({}, 1, true), h5features::exception,
                        [&](const auto &e) { return std::string(e.what()) == "features must have a non-zero size"; });

  BOOST_CHECK_EXCEPTION(h5features::features({}, 0, true), h5features::exception, [&](const auto &e) {
    return std::string(e.what()) == "features dimension must be greater than zero";
  });
...
BOOST_AUTO_TEST_CASE(test_validate_2) {
  BOOST_CHECK_EXCEPTION(h5features::features({1, 2, 1, 2, 3}, 3, true), h5features::exception, [&](const auto &e) {
    return std::string(e.what()) == "features size must be a multiple of dim";
  });
}
