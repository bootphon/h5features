...
BOOST_AUTO_TEST_CASE(test_validate_1) {
  BOOST_CHECK_EXCEPTION(h5features::times({0, 1}, {1, 0}, true), h5features::exception, [&](const auto &e) {
...
BOOST_AUTO_TEST_CASE(test_validate_2) {
  ...
  BOOST_CHECK_EXCEPTION(t1.validate(), h5features::exception, [&](const auto &e) {
    return std::string(e.what()) == "tstart must be lower or equal to tstop for all timestamps";
  });

  BOOST_CHECK_EXCEPTION(t2.validate(), h5features::exception, [&](const auto &e) {
    return std::string(e.what()) == "tstart must be lower or equal to tstop for all timestamps";
  });
}

BOOST_AUTO_TEST_CASE(test_validate_3) {
  BOOST_CHECK_EXCEPTION(h5features::times({2, 0}, {1, 1}, true), h5features::exception, [&](const auto &e) {
    return std::string(e.what()) == "timestamps must be sorted in increasing order";
  });
...
BOOST_AUTO_TEST_CASE(test_validate_4) {
  BOOST_CHECK_EXCEPTION(h5features::times({2, 0}, {1}, true), h5features::exception, [&](const auto &e) {
    return std::string(e.what()) == "tstart and tstop must have the same size";
  });
}

BOOST_AUTO_TEST_CASE(test_validate_5) {
  BOOST_CHECK_EXCEPTION(h5features::times({}, {}, true), h5features::exception,
                        [&](const auto &e) { return std::string(e.what()) == "timestamps must be non-empty"; });
}
...
BOOST_AUTO_TEST_CASE(test_get_format) {
  ...
  for (std::size_t s : {0, 3, 4, 5}) {
    BOOST_CHECK_EXCEPTION(h5features::times::get_format(s), h5features::exception,
                          [&](const auto &e) { return std::string(e.what()) == "invalid times dimension"; });
  }
}
...
BOOST_AUTO_TEST_CASE(test_indices_interval) {
  const h5features::times t{{0, 1, 2, 3, 4, 5}, {2, 3, 4, 5, 6, 7}};
