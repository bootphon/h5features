...
pybind::reader::reader(const std::string& filename,const std::string& group)
...
{
   // std::unique_ptr<pbitem> it = std::unique_ptr<pbitem>(
   //    dynamic_cast<pbitem*>(
   //       std::make_unique<h5features::item>(
   //          reader::read_item(name, ignore_properties=ignore_properties)).get()));
...
pybind::item pybind::reader::read_btw(const std::string& name, double start, double stop, bool ignore_properties)
{
   ...
   //    dynamic_cast<pbitem*>(
   //       std::make_unique<h5features::item>(
   //          reader::read_item(name, start, stop, ignore_properties=ignore_properties)).get()));
