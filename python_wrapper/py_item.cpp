...
{
   pybind11::class_<pbitem>(m, "Item", pybind11::buffer_protocol())
      .def(pybind11::init([](
         const std::string& name,
            ...
            auto item = pbitem(name, feats, tims, props, check);
            // auto finish = std::chrono::high_resolution_clock::now();
            // std::chrono::duration<double> elapsed = finish - start;
            // std::cout << "Elapsed time item: " << elapsed.count() << " s\n";
            return item;
         }))
      .def("__eq__", &pbitem::operator==, pybind11::is_operator(), "returns true if the two items instances are equal")
      .def("__ne__", &pbitem::operator!=, pybind11::is_operator(), "returns true if the two items instances are not equal")      
      .def("name", &pbitem::name, "returns the name of the item")
      .def("has_properties", &pbitem::has_properties, "Returns true if the item has attached properties, false otherwise")
      .def("dim", &pbitem::dim, "returns the dimension of a feature vector")
      .def("size", &pbitem::size, "returns the number of features vectors")
      .def("features",&pbitem::features)
      .def("times",&pbitem::times)
      .def("properties", &pbitem::properties)
      .def("properties_contains", &pbitem::properties_contains)
      .def("properties_erase", &pbitem::erase_properties)
      .def("properties_set", &pbitem::set_properties)
      ;
}
