...
public:
   ...
   static inline iterator begin_start(const std::vector<double>& v)
   {
      return {v.begin()};
   }

   static inline iterator end_start(const std::vector<double>& v)
   {
      return {v.end()};
   }

   static inline iterator begin_stop(const std::vector<double>& v)
   {
      return {v.begin() + 1};
   }

   static inline iterator end_stop(const std::vector<double>& v)
   {
      return {v.end() + 1};
   }

   iterator(std::vector<double>::const_iterator it)
      : m_it{it}
   {}
   ...
   inline difference_type operator-(iterator other) const
   {
      return (m_it - other.m_it) / 2;
   }
...
bool h5features::times::operator==(const times& other) const noexcept
{
   return this == &other or (
      m_format == other.m_format and m_data == other.m_data);
}
...
const std::vector<double>& h5features::times::data() const noexcept
{
   return m_data;
}
...
void h5features::times::validate() const
{
   ...
   switch(m_format)
   {
      ...
      default:  // format::interval
      {
         ...
         if(not (
               std::is_sorted(details::iterator::begin_start(m_data), details::iterator::end_start(m_data)) and
   ...
   switch(m_format)
   {
      case format::simple:
      {
         indices = {
            std::distance(
               m_data.begin(),
               std::lower_bound(m_data.begin(), m_data.end(), start)),
            std::distance(
               m_data.begin(),
               std::upper_bound(m_data.begin(), m_data.end(), stop))};
         break;
      }
      default:  // format::interval
      {
         indices = {
            std::distance(
               details::iterator::begin_start(m_data),
               std::lower_bound(
                  details::iterator::begin_start(m_data),
                  details::iterator::end_start(m_data),
                  start)),
            std::distance(
               details::iterator::begin_stop(m_data),
               std::upper_bound(
                  details::iterator::begin_stop(m_data),
                  details::iterator::end_stop(m_data),
                  stop))};
         break;
      }
   }
