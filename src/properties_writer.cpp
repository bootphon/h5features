...
class properties_writer_visitor {
public:
  ...
  // write a scalar
  template <class T> void operator()(const T &value) const { m_group.createAttribute<T>(m_name, value); }
  ...
  template <class T> void operator()(const std::vector<T> &value) const {
  ...
  void operator()(const std::shared_ptr<h5features::properties> &properties) const {
...
void h5features::details::write_properties(const h5features::properties &props, hdf5::Group &group, bool compress) {
  ...
  try {
    for (const auto &prop : props) {
      std::visit(properties_writer_visitor(group, prop.first, compress), prop.second);
    }
